...
async function ensurePricingTable() {
  await pool.execute(
    `CREATE TABLE IF NOT EXISTS interviewer_pricing (
      ...
      CONSTRAINT fk_interviewer_pricing_interviewer_backend
...
async function listInterviewerPricing(req, res, next) {
  try {
    await ensurePricingTable();
    const [rows] = await pool.execute(
...
async function listActiveInterviewerPricing(req, res, next) {
  try {
    await ensurePricingTable();
    const [rows] = await pool.execute(
